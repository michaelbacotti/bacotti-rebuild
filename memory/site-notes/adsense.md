...
- **Anti-pattern #92:** direct HTML edits to build.py-generated pages get wiped silently. Even with anti-pattern #89 in place, I (assistant) edited 3 build.py-generated pages in a single session without checking the source. **Fixed 2026-08-23** by shipping `skills/site-code-audit/scripts/check_source.py` (classifies any HTML as hand_crafted / md_source / inline_static / orphan) and `skills/site-code-audit/scripts/fix_thin.py` (auto-fixes at the SOURCE, not the HTML). Future rule: BEFORE any HTML edit, run `python3 skills/site-code-audit/scripts/check_source.py <file>` and obey the source_type warning.
